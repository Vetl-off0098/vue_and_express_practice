...
server.post('/cart', (req, res) => {
    ...
    fs.readFile(path, 'utf-8', (err, data) => {
        if(!err) {
            ...
            writing(path, JSON.stringify(newCart, null, ' '), res, {name: name, action: 'add'})
        } else {
...
server.put('/cart/:id', (req, res) => {
    ...
    fs.readFile(path, 'utf-8', (err, data) => {
        if(!err) {
            ...
            writing(path, JSON.stringify(newCart, null, ' '), res, {name: name, action: 'change'})
        } else {
            res.sendStatus(500, JSON.stringify({result: 0}))
        }
    })
...
server.delete('/cart/:id', (req, res) => {
    ...
    fs.readFile(path, 'utf-8', (err, data) => {
        if(!err) {
            ...
            writing(path, JSON.stringify(newCart, null, ' '), res, {name: name, action: 'del'})
        } else {
            res.sendStatus(500, JSON.stringify({result: 0}))
        }
    })
